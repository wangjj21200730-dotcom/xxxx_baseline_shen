    ...
    def debug_test_on_train(self, testloader):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(testloader, dynamic_ncols=True):
                ...
                # 注意：target_ids 中被 mask 的位置是 -100，直接 decode 会触发 tokenizer 溢出。
                pad_id = (
                    self.tokenizer.pad_token_id
                    if self.tokenizer.pad_token_id is not None
                    else 0
                )
                output_ids_for_decode = output_ids.clone()
                output_ids_for_decode = output_ids_for_decode.masked_fill(
                    output_ids_for_decode < 0, pad_id
                )
                gold_sents = self.tokenizer.batch_decode(
                    output_ids_for_decode, skip_special_tokens=True
                )
