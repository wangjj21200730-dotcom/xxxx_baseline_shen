...
class TestDatasetGRAM(Dataset):
    def __init__(
        ...
        # GRAM-C: 加载 item_id -> gcn_index 映射
        self.item_id_to_gcn_index = {}
        self.recent_k = getattr(args, 'recent_k', 5)
        if hasattr(args, 'item_id_to_gcn_index_path') and args.item_id_to_gcn_index_path:
            if os.path.exists(args.item_id_to_gcn_index_path):
                with open(args.item_id_to_gcn_index_path, 'r') as f:
                    self.item_id_to_gcn_index = json.load(f)
                logging.info(f"TestDatasetGRAM: Loaded {len(self.item_id_to_gcn_index)} item ID mappings")
    ...
    def construct_sentence(self):
        ...
        self.data["output"] = []
        self.data["user_id"] = []
        self.data["history_raw_ids"] = []  # GRAM-C: 保存原始 item IDs
    ...
    def get_item(self, idx):
        ...
        # GRAM-C: 计算 recent_item_ids
        if self.item_id_to_gcn_index:
            history_raw_ids = self.data["history_raw_ids"][idx]
            # 取最近 K 个物品（已经是时间顺序，取最后 K 个）
            recent_raw_ids = history_raw_ids[-self.recent_k:] if history_raw_ids else []
            
            # 映射到 GCN 索引
            recent_gcn_indices = []
            for raw_id in recent_raw_ids:
                gcn_idx = self.item_id_to_gcn_index.get(raw_id, 0)  # 未知映射到 0
                recent_gcn_indices.append(gcn_idx)
            
            # Padding: 不足 K 个时前面补 0
            while len(recent_gcn_indices) < self.recent_k:
                recent_gcn_indices.insert(0, 0)
            
            result["recent_item_ids"] = recent_gcn_indices
